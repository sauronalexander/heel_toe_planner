/*
 * Y_SS_Differential_Equa_Solver.c
 *
...
void Y_SS_Differential_Equa_Solver(double g, double h, double A4, double A3,
  double A2, double A1, double A0, double Constant, double y_init, double y_end,
  double tend, double *alpha, double *beta, double *b_gamma, double *fai, double
  *ita, double *Constant_prime, double *C1, double *C2)
{
  ...
  /* Solving Linear ODE for Y component during Single Support Phase */
  x[0] = A4;
  x[1] = A3;
  x[2] = A2;
  x[3] = A1;
  ...
  *Constant_prime = -Constant;
  ...
  dv1[1] = exp(-sqrt(g / h) * tend);
  dv1[3] = exp(sqrt(g / h) * tend);
  ...
  b_y_init[1] = (((((y_end - (-Constant)) - x[0] * rt_powd_snf(tend, 4.0)) - x[1]
                 ...
                 tend) - x[4];
  mldivide(dv1, b_y_init, y);
  *C1 = y[0];
  *C2 = y[1];
}

/* End of code generation (Y_SS_Differential_Equa_Solver.c) */
